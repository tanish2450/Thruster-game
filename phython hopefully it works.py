...
def reset_game():
    ...
    global stick_x, stick_y, angle, velocity_x, velocity_y, rotation_speed, obstacle_list, score, frame_count
    stick_x = WIDTH // 2
    stick_y = HEIGHT // 2
    angle = 0
...
reset_game()  # Start with fresh game variables
    ...
    if game_state == "menu":
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_RETURN:
                game_state = "playing"
                reset_game()  # Reset variables when starting a new game
        ...
        for obstacle in obstacle_list:
            ...
            pygame.draw.circle(screen, RED, (int(obstacle[0]), int(obstacle[1])), OBSTACLE_SIZE)
        ...
        draw_text(f"Game Over!", WIDTH // 2 - 60, HEIGHT // 3, RED, 40)
        draw_text(f"Score: {score}", WIDTH // 2 - 50, HEIGHT // 2, WHITE, 30)
        draw_text("Press ENTER to Retry", WIDTH // 2 - 120, HEIGHT // 1.5, GREEN, 30)
        pygame.display.flip()
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_RETURN:
                game_state = "playing"
                reset_game()  # Reset game properly
